...
class BoundingBox:
    ...
    def __post_init__(self) -> None:
        if not (-180.0 <= self.min_lon <= 180.0 and -180.0 <= self.max_lon <= 180.0):
            raise ValueError(f"longitude values must be in [-180, 180], got {self.min_lon}, {self.max_lon}")
        if not (-90.0 <= self.min_lat <= 90.0 and -90.0 <= self.max_lat <= 90.0):
            raise ValueError(f"latitude values must be in [-90, 90], got {self.min_lat}, {self.max_lat}")
